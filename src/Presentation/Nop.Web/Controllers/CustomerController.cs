...
public partial class CustomerController : BasePublicController
{
                ...
                case CustomerLoginResults.Successful:
                    {
                        ...
                        if (customer.UserType == UserType.SuptomerMerchant)
                            returnUrl = "/merchant";
                        else if (customer.UserType == UserType.SuptomerSupplier)
                            returnUrl = "/admin/supplier";
                        else
                            returnUrl = "/admin";
    ...
    public virtual async Task<IActionResult> PasswordRecoveryConfirmPOST(string token, string email, Guid guid, PasswordRecoveryConfirmModel model)
    {
        ...
        if (customer.UserType == UserType.SuptomerSupplier)
            model.ReturnUrl = "/supplier";
        else if (customer.UserType == UserType.SuptomerMerchant)
            model.ReturnUrl = "/merchant";
        return View(model);
    }
    ...
    //available even when navigation is not allowed
    //[CheckAccessPublicStore(ignore: true)]
    //public virtual async Task<IActionResult> Register(string returnUrl, UserType userType)
    //{
    ...
    #region Suptomer Register

    [CheckAccessPublicStore(ignore: true)]
    public virtual async Task<IActionResult> Register_old(string returnUrl, UserType userType)
    {
        //check whether registration is allowed
        if (_customerSettings.UserRegistrationType == UserRegistrationType.Disabled)
            return RedirectToRoute("RegisterResult", new { resultId = (int)UserRegistrationType.Disabled, returnUrl });
    ...
    [CheckAccessPublicStore(ignore: true)]
    public virtual IActionResult Register(string returnUrl, UserType userType)
    {
        var model = new SuptomerRegisterModel
        {
            UserType = userType
        };
    ...
    [ValidateHoneypot]
    [CheckAccessPublicStore(ignore: true)]
    public virtual async Task<IActionResult> Register(SuptomerRegisterModel model, string returnUrl)
    {
        var customer = await _workContext.GetCurrentCustomerAsync();
        ...
        if (ModelState.IsValid)
        {
            ...
            if (registrationResult.Success)
            {
                ...
                if (customer.UserType == UserType.SuptomerSupplier)
                {
                    var vendor = new Vendor { Name = customer.FirstName, Email = customer.Email, Active = true };
    ...
    public virtual async Task<IActionResult> AccountActivation(string token, string email, Guid guid)
    {
        ...
        {
            model.Result = await _localizationService.GetResourceAsync("Account.AccountActivation.AlreadyActivated");
            model.ReturnUrl = Url.RouteUrl("Login");
            return View(model);
        }
