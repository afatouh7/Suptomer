...
function checkOverriddenStoreValue(obj, selector) {
  ...
  if (!$(obj).is(":checked")) {
    ...
    $.each(elementsArray, function (key, value) {
      var kenoduiElement =
        $(value).data("kendoNumericTextBox") ||
        $(value).data("kendoMultiSelect");
...
//open close admin search filter
...
function ToggleSearchBlockAndSavePreferencesAndCloseSideMenu() {
  //open card search
  $(".card-search").find(".search-body").slideToggle();
  const openSearch = $(".card-search")
    .children(".card-body")
    .children(".search-row");
    $("#search-rslt").removeClass();
    $("#search-col").removeClass();
  if (openSearch.hasClass("opened")) {
       $("#search-rslt").addClass("col-sm-12 col-md-9 col-lg-10");
       $("#search-col").addClass("col-sm-12 col-md-3 col-lg-2 bg-light_theme d-block");
    saveUserPreferences(
      rootAppPath + "admin/preferences/savepreference",
      openSearch.attr("data-hideAttribute"),
      true
    );
  } else {
    //reset classes
    $("#search-rslt").addClass("col-sm-12");
  ...
  $(".card-search").find(".search-row").toggleClass("opened");
...
function switchViewAction(){
  console.log('switch view cooming featuer')
...
$(document).ready(function () {
   ...
   $(".filter_icon").click(ToggleSearchBlockAndSavePreferencesAndCloseSideMenu);
   $("#switchView").on("click",()=>{switchViewAction();});
});

//Nivo Slider for prouct image preview
    // Initialize Nivo Slider for each product's image slider
    $(document).ready(function () {
