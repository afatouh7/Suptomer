...
public partial class OrderModelFactory : IOrderModelFactory
{
        ...
        if (order.ShippingStatus != ShippingStatus.ShippingNotRequired)
        {
            ...
            else if (order.PickupAddressId.HasValue && await _addressService.GetAddressByIdAsync(order.PickupAddressId.Value) is Address pickupAddress)
            {
                model.PickupAddress = new AddressModel
                {
                    ...
                    StateProvinceName = await _stateProvinceService.GetStateProvinceByAddressAsync(pickupAddress) is Core.Domain.Directory.StateProvince stateProvince
    ...
    public virtual Task<PaymentSearchModel> PreparePaymentSearchModelAsync(PaymentSearchModel searchModel)
    {
        if (searchModel == null)
    ...
    public virtual async Task<PaymentGridVM> PreparePaymentListModelAsync(PaymentSearchModel searchModel)
    {
        if (searchModel == null)
            throw new ArgumentNullException(nameof(searchModel));
        ...
        var store = await _storeContext.GetCurrentStoreAsync();
        var orders = (await _orderService.SearchOrdersAsync(storeId: store.Id,
            customerId: customer.Id)).ToPagedList(searchModel);

        var model = new PaymentGridVM().PrepareToGrid(searchModel, orders, () =>
        {
            return orders.Select(order =>
            {
                var orderTotalInCustomerCurrency = _currencyService.ConvertCurrency(order.OrderTotal, order.CurrencyRate);

                var orderItems = _orderService.GetOrderItemsAsync(order.Id).GetAwaiter().GetResult();
                var productsAndQuantities = new StringBuilder();

                foreach (var orderItem in orderItems)
                {
                    var product = _productService.GetProductByIdAsync(orderItem.ProductId).GetAwaiter().GetResult();
                    var productName = _localizationService.GetLocalizedAsync(product, x => x.Name).GetAwaiter().GetResult();
                    var vendorName = (_vendorService.GetVendorByIdAsync(product.VendorId).GetAwaiter().GetResult())?.Name ?? string.Empty;

                    productsAndQuantities.AppendLine($"({orderItem.Quantity}) {productName} from {vendorName}");
                }

                return new PaymentListVM
                {
                    Date = (_dateTimeHelper.ConvertToUserTimeAsync(order.CreatedOnUtc, DateTimeKind.Utc).GetAwaiter().GetResult()).ToString("dd/MM/yyyy"),
                    OrderNumber = order.CustomOrderNumber,
                    Amount = _priceFormatter.FormatPriceAsync(orderTotalInCustomerCurrency, true, order.CustomerCurrencyCode, false, (_workContext.GetWorkingLanguageAsync().GetAwaiter().GetResult()).Id).GetAwaiter().GetResult(),
                    Id = order.Id,
                    Products = productsAndQuantities.ToString()
                };
            });
        });
