...
public partial class CatalogModelFactory : ICatalogModelFactory
{
    ...
    {
      //  var picture = await _pictureService.GetPictureByIdAsync(currentCategoryId);
        //var picturePath = await _pictureService.GetThumbLocalPathAsync(picture, 0, false);
        ...
        foreach (var category in cachedCategoriesModel)
        {
            // Get the index of the last directory separator (\ or /) in PIcutureUrl
            int lastSeparatorIndex = category.PIcutureUrl.LastIndexOfAny(new[] { '\\', '/' });

            // If a separator is found, extract the file name (including extension) from PIcutureUrl
            if (lastSeparatorIndex != -1)
            {
                category.PIcutureUrl = category.PIcutureUrl.Substring(lastSeparatorIndex + 1);
            }
        }
    ...
    public virtual async Task<List<CategorySimpleModel>> PrepareCategorySimpleModelsAsync(int rootCategoryId, bool loadSubCategories = true)
    {
        ...
        foreach (var category in categories)
        {
            var categoryModel = new CategorySimpleModel
            {
                ...
                PIcutureUrl = await _pictureService.GetThumbLocalPathAsync(await _pictureService.GetPictureByIdAsync(category.PictureId), 0,true),
    ...
    public virtual async Task<CatalogProductsModel> PrepareAllProductsModelAsync(CatalogProductsCommand command)
    {
        ...
        //products
        var products = await _productService.GetProductsAsync(storeId: currentStore.Id,
            pageIndex: command.PageNumber - 1,
            pageSize: command.PageSize);
