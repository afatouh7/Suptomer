...
public partial class ProductService : IProductService
{
    ...
    private readonly IRepository<SupplierContract> _supplierContract;
        ...
        LocalizationSettings localizationSettings)
    {
        ...
        _supplierContract = supplierContract;
    ...
    public virtual async Task<IPagedList<Product>> GetProductsAsync(int storeId = 0, int pageIndex = 0, int pageSize = int.MaxValue)
    {
        var query = from p in _productRepository.Table
                    where p.Published &&
    ...
    public async Task<List<ProductsAboutToExpireDto>> GetTop3AboutToExpireProducts(int vendorId)
    {
        var last30Dayes = DateTime.UtcNow.AddDays(-30).Date;

        var result = await
            (from p in _productRepository.Table
             where !p.Deleted &&
                  p.CreatedOnUtc.Date <= last30Dayes &&
                  p.VendorId == vendorId
             orderby p.CreatedOnUtc
             select new ProductsAboutToExpireDto
             {
                 Name = p.Name,
                 Quantity = p.UseMultipleWarehouses
                    ? _productWarehouseInventoryRepository.Table.Where(pwi => pwi.ProductId == p.Id).Sum(pwi => pwi.StockQuantity)
                    : p.StockQuantity
             }).Take(3).ToListAsync();

        var totalNumberOfProducts = await _productRepository.Table
            .Where(p => !p.Deleted && p.VendorId == vendorId)
            .Select(p => p.UseMultipleWarehouses
                ? _productWarehouseInventoryRepository.Table.Where(pwi => pwi.ProductId == p.Id).Sum(pwi => pwi.StockQuantity)
                : p.StockQuantity)
            .SumAsync(p => p);
        ...
        if (totalNumberOfProducts > 0)
            result.ForEach(p =>
                p.QuantityPercentage = (int)Math.Round(((decimal)p.Quantity / totalNumberOfProducts) * 100));

        return result;
    }

    public Task<ProductCategory> GetProductCategoryByProductIdAsync(int productId)
    {

        var query = from p in _productCategoryRepository.Table
                    where p.ProductId == productId
                    select p;
        return (Task<ProductCategory>)query;
    }

    public async Task AddSuplierContractAsync(SupplierContract supplierContract)
    {
        if (supplierContract is null)
            throw new ArgumentNullException(nameof(supplierContract));

      

         _supplierContract.Insert(supplierContract);
    }

    public async Task<IList<SupplierContract>> GetAllContractForMerchantByEmail(string email)
    {
        
     var contracts=   _supplierContract.Table.Where(x=>x.Email==email).ToList();
        return contracts;
...
public class ProductsAboutToExpireDto
{
