...
namespace Nop.Services.Shipping
{
    ...
    public partial class ShippingService : IShippingService
    {
        ...
        /// </returns>
        public virtual async Task<IList<Warehouse>> GetAllWarehousesAsync(int? userId, string name = null)
        {
            var warehouses = await _warehouseRepository.Table
        ...
        public virtual async Task InsertWarehouseAsync(Warehouse warehouse)
        {
            ...
            warehouse.UserId = currentCustomer.Id;
        ...
        /// </returns>
        public virtual async Task<Warehouse> GetNearestWarehouseAsync(int? userId, Address address, IList<Warehouse> warehouses = null)
        {
            warehouses ??= await GetAllWarehousesAsync(userId);
            ...
            Address shippingAddress, int storeId)
        {
            ...
            foreach (var sci in cart)
            {
                ...
                if (_shippingSettings.UseWarehouseLocation)
                {
                    ...
                    {
                        var warehouseOwner = await _customerService.GetCustomerByVendorId(product.VendorId);
                        ...
                        warehouse = await GetNearestWarehouseAsync(warehouseOwner?.Id, shippingAddress, allWarehouses);
                    }
                    else
                    {
